...
- =gptel-plus-tokens-in-output=: Set to =250=, based on my own intuition. You can set this to any other value or, if you are much more , to =gptel-max-tokens=, ensuring that the estimate will never underestimate the actual output cost.
...
/NB: currently, this feature works for Anthropic models only./

In addition to providing an /ex ante/ cost estimate, =gptel-plus= can also calculate the actual cost of a request /ex post/. This is done by temporarily setting =gptel-log-level= to =info=, which prompts =gptel= to log the request and response data to the =*gptel-log*= buffer. =gptel-plus= then parses this log to extract the exact number of input and output tokens, and uses this information to calculate the precise cost of the request. The result is then displayed in the echo area.

To disable this feature, set the user option =gptel-plus-calculate-cost= to =nil=.
...
- =gptel-plus-restore-file-context=: Restore a previously saved context.

Context is stored as file-local variables in Markdown files and as properties in Org files.
