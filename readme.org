...
The development repository is on [[https://github.com/benthamite/gptel-plus][GitHub]].
...
- *Session cost accumulation.*  Track cumulative spend across a session, with a command or header line indicator to show the running total.

- *Region-based context in cost estimation.*  Context added via ~gptel-context--add-region~ is currently not included in the /ex ante/ cost estimate.  The cost should account for region-based context as well as file-based context.

- *Restricted and branching conversation awareness.*  When using gptel's restricted context (limited to an Org heading) or branching conversations, the /ex ante/ cost estimate should reflect only the portion of the buffer that will actually be sent.

- *Auto-restore context on mode activation.*  When ~gptel-mode~ is automatically enabled (via the mode activation hooks), any saved context in the file could be restored automatically, removing the need to call ~gptel-plus-restore-file-context~ manually.

- *Show actual cost in header line.*  After a request completes, the header line could display the /ex post/ cost alongside the /ex ante/ estimate, closing the feedback loop between estimated and actual spend.
