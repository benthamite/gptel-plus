...
:PROPERTIES:
:CUSTOM_ID: h:programmatic-cost-api
:END:

#+findex: gptel-plus-compute-cost

The cost estimation features described above ([[#h:cost-estimation][Cost estimation]]) work automatically for interactive ~gptel-send~ calls.  For programmatic use via ~gptel-request~, =gptel-plus= provides a separate mechanism.
